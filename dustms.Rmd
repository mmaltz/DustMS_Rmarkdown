...
mds.uni<-metaMDS(unifrac, k=2)
mds.uniwt<-metaMDS(unifrac_wt, k=2)
...
nmds.uni<-nmds.uni[match(grps$SampleID,rownames(nmds.uni)),]%>%
  cbind(grps) # then add that df, left_join would have similar effect
nmds.uni
...
```
Separates well by elevation, within each elevation the months are grouped together for the most part. 
