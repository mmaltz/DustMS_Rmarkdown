...
Compare samples with PERMANOVA and NMDS    
...
otu.mat<-otu.comm%>%dplyr::select(-SampleID)

otu.grp<-otu.comm%>%left_join(map6, by='SampleID')

##dissimiarlity
uni.dist<-B16S%>%dplyr::select(OJ514)
dist<-vegdist(otu.mat, method='jaccard', na.rm=TRUE)
```  
...
#permanova
month.adonis<-adonis2(dist~Month*Elevation, data=otu.grp)
month.adonis
```
