...
UniFrac is a measure of B-diversity that uses phylogenetic information to compare community samples. Use with multivariate methods like principal coordinates (PCoA), NMDS, etc to explain differences among communities. Measured as the amount of evolutionary history that is unique to each group - the fraction of branch length in a phylogenetic tree associated with  ONLY the focal sample in pairwise comparison. With sequence data UniFrac values can be invfluencd by the number of sequences/sample and can use sequence jackknifing (Lozupone et al 2011. ISME).Use to compare phylogenetic lineages between groups, to cluster samples. 
...
otu.comm[is.na(otu.comm)]<-0 #replace NAs with 0
...
```{r nmds plot df, eval=FALSE, include=FALSE}
...
# save dataframes - reproducible with same 'set.seed()' 
write.csv(nmds.uni, 'data/nmds.unifrac.csv', row.names=FALSE)
write.csv(nmds.uniwt, 'data/nmds.unifracwt.csv', row.names=FALSE)
```
Plot ordination 
```{r plot nmds with Year test 2}
nmds.uni<-read.csv('data/nmds.unifrac.csv')%>% # correct data types for plotting
  transform(Elevation = as.numeric(as.integer(Elevation)), Month = as.factor(Month), Year = as.factor(Year))
nmds.uniwt<-read.csv('data/nmds.unifracwt.csv')%>% # correct data types for plotting
  transform(Elevation = as.numeric(as.integer(Elevation)), Month = as.factor(Month), Year = as.factor(Year))
...
# can be done like this below OR on ethe dataframe when read in initially so it applies to everything (better option, see above)
...
ggplot(nmds.uniwt, aes(NMDS1, NMDS2))+
  ...
  # but also year cannot be given a continuous scale_color_gradient
  #instead try scale_color_manual to be able to provide the colors for each year

# manually assign discrete color scale
ggplot(nmds.uniwt, aes(NMDS1, NMDS2))+
  geom_point(aes(color=Year, shape=Month), size=4)+
  scale_color_manual(values=c('darkslateblue','gold'))
```
```{r}
...
ggplot(df_shapes, aes(0, 0, shape = shape)) +
  geom_point(aes(shape = shape), size = 3, fill = 'red') +
...
# we hve to assign a different shape
# fill = Year, shape = 21
ggplot(nmds.uni, aes(NMDS1, NMDS2))+
  geom_point(aes(fill=Year), shape = 21,color='black', size=3)+
  theme_bw()
# shape = 21 is the filled circle

# shape = Month
ggplot(nmds.uni, aes(NMDS1, NMDS2))+
  geom_point(aes(fill=Year, shape=Month),color='black', size=3)+
...
```  
```{r}
# add ellipses - 95% CI multivariate normal
ggplot(nmds.uni, aes(NMDS1, NMDS2))+
  geom_point(aes(fill=Year, shape = Month), color='black', size=3)+
  scale_shape_manual(values=c(21, 22, 24))+ # the values correspond to the shapes above
  scale_fill_manual(values=c('black','white'))+
  guides(fill = guide_legend(override.aes=list(shape=21)))+ 
  theme_bw()+
  annotate('text', x=0.4, -0.28, label=paste0('Stress = ',round(mds.uni$stress,2)))+
  stat_ellipse(aes(group=Year, lty=Year), type='norm')

```
```{r}
# look at elevation and time
nmds.plot<-nmds.uni%>%transform(Elevation = as.ordered(Elevation))

ggplot(nmds.plot, aes(NMDS1, NMDS2))+
  geom_point(aes(fill=Year, shape = Elevation), color='black', size=3)+
  scale_fill_manual(values=c('black','white'))+
  guides(fill = guide_legend(override.aes=list(shape=21)))+ 
  annotate('text', x=0.3, -0.25, label=paste0('Stress = ',round(mds.uni$stress,2)))+
  scale_shape_manual(values=c(21, 22, 23, 24))+
  theme_bw()#+
   # the values correspond to the shapes above
...
** Run PCoA and examine the results**  
```{r run PCoa, include=TRUE, eval=TRUE}
#PCoA - unconstrained, euclidean
pcoa.uni<-capscale(unifrac~1,data=grps, comm=otu.comm[,-1])
head(summary(pcoa.uni))
# this returns a lot of stuff
# the importance of components tells you the total variation explained by your axes 
# this is your indication of goodness of fit rather than stress in the nmds
# the species scores are how each species (otu) fits in the ordination space
```
```{r plot pcoa}
# the site scores at the bottom are the weighted sums of species scores
# this means these are the SampleID points in ordination space, use that to plot as before
pcoa.df<-data.frame(scores(pcoa.uni, display='sites'))# site scores correspond to the SampleID
# bind with grouping data, in order
pcoa.df<-pcoa.df[match(grps$SampleID,rownames(pcoa.df)),]%>%cbind(grps)%>% # match rows and bind
  transform(Elevation = as.ordered(Elevation), Year = as.factor(Year))

ggplot(pcoa.df, aes(MDS1, MDS2))+
  geom_point(aes(fill=Year, shape = Elevation), color='black', size=3)+
  scale_fill_manual(values=c('black','white'))+
  guides(fill = guide_legend(override.aes=list(shape=21)))+ 
  scale_shape_manual(values=c(21, 22, 23, 24))+
...
## can also add your otus - BUT there were thousands so that will never plot
#first identify which ones are important and related to the ordination

# can do this with envfit
env.df<-grps%>%dplyr::select(Year, Month, Elevation, Site)%>%transform(Elevation = as.numeric(Elevation), Year=as.factor(Year))
env.fact<-envfit(pcoa.uni, env.df, permutations =1000, na.rm=TRUE)# returns significance and R2 for correlations with environemntal vars
# should be similar to nmds results unless the ordination has poor fit
env.fact
# can also test with individual species or taxonomic groups butthere are way too many otus for this to produce a very clear result in this case

```  
```{r}
scores(pcoa.uni, display='species') # pull species socres
```  

#### Constrained ordination  
The prior ordinations were unconstrained, meaning they reflect the true variation of the data as defined by the species compoisiton. In that case you can look at correlations between coposition and environmental variables or species. But these are more exploratory methods, and to determine which variables or species best explain the differences between groups, constrained ordination may be appropriate.  

A distance-based reducndancy analysis is the constrained version of the PCoA. The dbrda tests which variables ebst explain the observed variation. Thus, the ordination will be slightly different. It is designed to work with distance data, like the unifrac.

```{r include=FALSE, eval=FALSE}
# constrained ordination - explaint he diffrerences
##dbrda
dbrda.uni<-capscale(unifrac~Elevation+Month,data=grps)
head(summary(dbrda.uni))

# now the CAP axes shown are constrained
# see provides biplot scores for contraining variables
```
```{r}
...
```
#### Identify relevant species/otus in ordination  

When you have an unconstrained ordination and you see variation between your groups, you may be interested in which species contribute to these differences and/or how species composition turnsover in multivariate space.  

One way to look at this is to examine how well your species (or otus, or any taxonomy etc) are correlated with an ordination and plot vectors.


#### Relating  community composition to the environment 
